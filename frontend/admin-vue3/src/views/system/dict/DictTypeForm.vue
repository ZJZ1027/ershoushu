<template>
  ...
  >
    <a-form ref="formRef" :model="formData" :rules="formRules" layout="vertical">
      <a-form-item field="name" label="字典名称" required>
        <a-input v-model="formData.name" placeholder="请输入字典名称" allow-clear />
      </a-form-item>
...
const open = async (type: string, id?: number) => {
  ...
  // 上游模板一律用「新增 / 修改」这种无主语标题；业务页早就改成「新增人员」这种带对象的说法，
  // 系统页跟上——弹窗标题是用户唯一的定位信息，光说动作等于什么都没说。
  dialogTitle.value = type === 'create' ? '新增字典类型' : '编辑字典类型'
  formType.value = type
  resetForm()
  // 修改时，设置数据
  if (id) {
...
const handleBeforeOk = async () => {
  ...
  formLoading.value = true
  try {
    // 新增时 id 为空、createTime 由后端生成，与 DictTypeVO 只是部分重叠，需先转 unknown
