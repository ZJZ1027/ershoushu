...
// 字典数据的 colorType 到 Arco a-tag color 的映射
const EP_TYPE_TO_ARCO_COLOR: Record<string, string> = {
  success: 'green',
  info: 'gray',
  warning: 'orange',
  danger: 'red',
        ...
        >
          {dictOptions.map((dict: DictDataType) => {
            if (valueArr.value.includes(dict.value)) {
              // 优先用自定义 hex 背景色；否则按字典的 colorType 映射到 Arco 预设色，默认 arcoblue
              const hex = dict?.cssClass && isHexColor(dict?.cssClass) ? dict.cssClass : ''
              const color = hex || EP_TYPE_TO_ARCO_COLOR[dict?.colorType + ''] || 'arcoblue'
              return <ATag color={color}>{dict?.label}</ATag>
            }
          })}
        </div>
      )
