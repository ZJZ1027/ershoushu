...
/* 组件兜底：后端菜单指向的前端视图不存在时（如已删除的 infra/* 页面），
 * 回退到 404 而不是渲染一个空 component 导致白屏 */
export const RouteView404 = () => import('@/views/Error/404.vue')
/** 把后端菜单的 component/path 解析成前端模块；找不到时回退到 404 并告警 */
const resolveRouteComponent = (route: AppCustomRouteRecordRaw, modulesRoutesKeys: string[]) => {
  const index = route?.component
    ? modulesRoutesKeys.findIndex((ev) => ev.includes(route.component as string))
    : modulesRoutesKeys.findIndex((ev) => ev.includes(route.path))
  if (index === -1) {
    console.warn(
      `[router] 菜单「${route.name}」对应的前端视图未找到：${route.component || route.path}，已回退到 404 页`
    )
    return RouteView404
  }
...
export const generateRoute = (routes: AppCustomRouteRecordRaw[]): AppRouteRecordRaw[] => {
  ...
  for (const route of routes) {
    ...
    if (!route.children && route.parentId == 0 && route.component) {
      ...
      childrenData.component = resolveRouteComponent(route, modulesRoutesKeys)
      ...
      if (route.children?.length) {
        ...
        data.component = resolveRouteComponent(route, modulesRoutesKeys)
      }
      if (route.children) {
